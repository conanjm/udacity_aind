...
def alpha_beta_search(state, play_id, depth = 3):
    ...
    for action in state.actions():
        value = min_value(state.result(action), play_id, alpha, beta, depth - 1)
...
def min_value(state, play_id, alpha, beta, depth):
        ...
        value = float("inf")
        for action in state.actions():
            value = min(value, max_value(state.result(action), play_id, alpha, beta, depth - 1))
            if value <= alpha:
                return value
...
def max_value(state, play_id, alpha, beta, depth):
        if state.terminal_test():
            return state.utility(play_id)
        ...
        for action in state.actions():
            value = max(value, min_value(state.result(action), play_id, alpha, beta, depth - 1))
            if value >= beta:
                return value
