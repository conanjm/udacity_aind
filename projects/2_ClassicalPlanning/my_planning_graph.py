...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        action_one = [~e in actionA.effects for e in actionB.effects]
        other_action = [~e in actionB.effects for e in actionA.effects]

        return any(action_one + other_action)
    ...
    def _interference(self, actionA, actionB):
        ...
        """
        # TODO: implement this function
        action_one = [~e in actionA.preconditions for e in actionB.effects]
        other_action = [~e in actionB.preconditions for e in actionA.effects]
        
        return any(action_one) or any(other_action)
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        """
        # TODO: implement this function
        parent_layer_a = any(self.parent_layer.is_mutex(itemA,itemB) \
            for itemA in actionA.preconditions \
            for itemB in actionB.preconditions) 
        parent_layer_b = any(self.parent_layer.is_mutex(itemB, itemA) \
            for itemA in actionA.preconditions \
            for itemB in actionB.preconditions)
        
        return parent_layer_a or parent_layer_b
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        """
        # TODO: implement this function
        parent_layer_a = all(self.parent_layer.is_mutex(actionA,actionB) \
            for actionA in self.parents[literalA] \
            for actionB in self.parents[literalB])
        parent_layer_b = all(self.parent_layer.is_mutex(actionB, actionA) \
            for actionA in self.parents[literalA]
            for actionB in self.parents[literalB])
        
        return parent_layer_a and parent_layer_b
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        level_sum = 0
        for g in self.goal:
            for id_x,layer in enumerate(self.literal_layers):
                if g in layer:
                    level_sum += id_x
                    break
